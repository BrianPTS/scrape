  ...
  const scheduledTask = async () => {
    ...
    try {
      ...
      if (result.success && result.csv) {
        ...
        if (currentSettings.uploadToSync) {
          ...
          if (uploadResult.success) {
            ...
            await createErrorLog({
              operation: 'CSV_SCHEDULER_UPLOAD',
              errorType: 'DATABASE_ERROR',
              message: uploadResult.message || 'Unknown upload error',
              functionName: 'scheduled_csv_upload',
              timestamp: new Date()
            });
          }
        } else {
        ...
        // Log error to database
        await createErrorLog({
          operation: 'CSV_SCHEDULER_GENERATION',
          errorType: 'DATABASE_ERROR',
          message: result.message || 'Unknown generation error',
          functionName: 'scheduled_csv_generation',
          timestamp: new Date()
        });
      }
    } catch (error) {
      ...
      // Log error to database
      await createErrorLog({
        operation: 'CSV_SCHEDULER_TASK',
        errorType: 'DATABASE_ERROR',
        message: errorMessage,
        stack: error instanceof Error ? error.stack : undefined,
        functionName: 'scheduled_csv_task',
        timestamp: new Date()
      });
    }
  };
